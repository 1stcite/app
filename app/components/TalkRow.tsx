...
function ChairIcon({ filled, color }: { filled: boolean; color: string }) {
  return (
    <svg className="w-5 h-5" viewBox="-19.82 0 122.88 122.88"
      fill={filled ? color : "none"} stroke={filled ? "none" : "currentColor"} strokeWidth={filled ? 0 : 6}>
...
export default function TalkRow({ poster, isStarred, onToggleStar }: TalkRowProps) {
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            {poster.author && <span className="text-sm text-gray-500 truncate">{poster.author}</span>}

            {/* Icon toggles inline with author */}
            <div className="flex items-center gap-0.5 shrink-0">
              <div className="relative">
                <Toast msg={starToast.msg} />
                <button
                  onClick={() => { onToggleStar(poster.id); if (!isStarred) starToast.show("Interested"); }}
                  className={`p-1 rounded-md transition-colors ${isStarred ? "text-amber-500" : "text-gray-400 hover:text-amber-500"}`}
                  title="Interested"
                >
              ...
              <div className="relative">
                ...
                <button
                  onClick={() => { toggleAttend(); if (!attended) { attendToast.show("Attend"); if (!isStarred) onToggleStar(poster.id); } }}
                  className={`p-1 rounded-md transition-colors ${attended ? "text-emerald-600" : "text-gray-400 hover:text-emerald-600"}`}
                  title="Attend"
                >
              ...
              <div className="relative">
                ...
                <button
                  onClick={() => { toggleSave(); if (!saved) libraryToast.show("Save to Library"); }}
                  className={`p-1 rounded-md transition-colors ${saved ? "text-blue-600" : "text-gray-400 hover:text-blue-600"}`}
